...
namespace OT::perspective {
        ...
        cv::Mat getPerspectiveMatrix(std::vector<cv::Point2f> &fourPoints,
            ...
            auto maxWidth = (std::float_t) std::max(widthA, widthB);

            auto heightA = cv::norm(tr - br);
            auto heightB = cv::norm(tl - bl);
            auto maxHeight = (std::float_t)std::max(heightA, heightB);

            // Set the size as these dimensions.
            size.width = (std::int32_t) maxWidth;
            size.height = (std::int32_t) maxHeight;
        ...
        void extractFourPoints(const std::vector<std::int64_t> &ints,
            ...
            for (size_t i = 0; i < 4; i++) {
                points.emplace_back(ints[i*2], ints[i*2+1]);
            }
        }
    }
