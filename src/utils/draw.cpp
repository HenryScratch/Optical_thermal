...
namespace OT::utils::draw {
        ...
        void drawTrajectory(const cv::Mat& img,
            ...
            for (size_t i = 0; i < trajectory.size() - 1; i++) {
                line(img, trajectory[i], trajectory[i+1], color, 1, cv::LineTypes::LINE_AA, 0);
            }
        }
        ...
        void contourShow(const std::string& drawingName,
            ...
            for (size_t i = 0; i < contours.size(); i++) {
                cv::drawContours(drawing,
                                 ...
                                 0,
                                 cv::Point());
                OT::utils::draw::drawBoundingRect(drawing, boundingRect[i]);
            }
